...
exports.placeOrder = onCall(async (request) => {
  ...
  try {
    ...
    // Re-throw HttpsErrors as-is (they're already the intended,
    // user-facing message). Anything else is a bug we didn't
    // anticipate — log the real message/stack so it's visible in
    // Cloud Functions logs instead of vanishing behind the generic
    // "internal" error the client sees.
    if (err instanceof HttpsError) throw err;
    console.error("placeOrder failed:", err);
    throw new HttpsError("internal", "Could not place order. Please try again.");
  }
});
